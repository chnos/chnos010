...
uint Sheet08_Internal_SetBuffer(UI_Sheet *sheet, void *vram, uint xsize, uint ysize, uint bpp)
{
	...
	if(vram == Null){
		sheet->vram = System_Memory_Allocate(xsize * ysize * 1);
	...
	sheet->vramsize = xsize * ysize * 1;
	sheet->Config_Functions = &Sheet08_Config_Functions;
	sheet->IsVisiblePixel = &Sheet_Internal_IsVisiblePixel_Invalid;
	sheet->flags.bit.using_invcol = False;

	sheet->Drawing.Fill_Rectangle = &Sheet08_Drawing_Fill_Rectangle;
	sheet->Drawing.Put_String = &Sheet08_Drawing_Put_String;
	sheet->Drawing.Draw_Point = &Sheet08_Drawing_Draw_Point;
...
uint Sheet08_Config_Functions(UI_Sheet *sheet)
{
	...
	if(sheet->parent->bpp == 8){
		sheet->RefreshSheet = &Sheet08_Internal_RefreshSheet_To_08;
	} else if(sheet->parent->bpp == 16){
		sheet->RefreshSheet = &Sheet08_Internal_RefreshSheet_To_16;
	} else if(sheet->parent->bpp == 32){
		sheet->RefreshSheet = &Sheet08_Internal_RefreshSheet_To_32;
	} else{
		#ifdef CHNOSPROJECT_DEBUG_SHEET
...
uint Sheet08_Internal_RefreshSheet_To_16(UI_Sheet *sheet, int px0, int py0, int px1, int py1)
{
	...
	for(y = p.y; y <= r.y; y++){
		for(x = p.x; x <= r.x; x++){
			if(sheet->parent->map[y * sheet->parent->size.x + x] == (uint)sheet){
				((ushort *)sheet->parent->vram)[y * sheet->parent->size.x + x] = RGB_08_To_16(((uchar *)sheet->vram)[(y - sheet->location.y) * sheet->size.x + (x - sheet->location.x)]);
			}
		}
	}
...
uint Sheet08_Internal_RefreshSheet_To_32(UI_Sheet *sheet, int px0, int py0, int px1, int py1)
{
	...
	for(y = p.y; y <= r.y; y++){
		for(x = p.x; x <= r.x; x++){
			if(sheet->parent->map[y * sheet->parent->size.x + x] == (uint)sheet){
				((uint *)sheet->parent->vram)[y * sheet->parent->size.x + x] = RGB_08_To_32(((uchar *)sheet->vram)[(y - sheet->location.y) * sheet->size.x + (x - sheet->location.x)]);
			}
		}
	}
...
{
	if(((uchar *)(sheet->vram))[(py - sheet->location.y) * (int)sheet->size.x + (px - sheet->location.x)] == sheet->invcol){
		return False;
	}
